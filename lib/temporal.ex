...
defmodule AshPostgres.Temporal do
  ...
  # table name and before the ` AS <alias>` that Ecto emits.
  #
  # ⭐ The upper is `NULL` for an unbounded portion — the open upper, and see the
  # moduledoc for why not `'infinity'` — or a second bound `$2` when `as_of` named a
  # period. Postgres carves on the INTERSECTION of the portion with each row's period,
  # so a bounded portion leaves the row's validity intact either side of it.
  defp splice_for_portion_of(sql, kind, attribute, upper?) do
    ...
    to =
      if upper?, do: "$2::timestamptz", else: "NULL"
