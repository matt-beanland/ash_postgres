...
defmodule AshPostgres.TemporalTest do
  ...
  # ⭐ A range-valued `as_of` names the PORTION a write applies to rather than the instant
  # it opens at. Postgres carves on the intersection of the portion with each row's period,
  # so a bounded portion leaves validity intact on both sides — which is `FOR PORTION OF`
  # with a real `TO` bound instead of `TO NULL`.
  describe "an as_of naming a period" do
    ...
    # ⛔ `TO NULL` must survive: an instant-valued `as_of` still means "from here onward",
    # and emitting a bounded portion for it would silently close an open version.
